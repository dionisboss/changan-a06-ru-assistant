...
**Version:** 1.0.5 · Distributed free of charge.
...
- **Online path (optional)** — free‑form questions, chit‑chat, weather, and commands the offline mapper
  does not know (or the car refused) go to an LLM backend (`BACKEND` in `RuBridge.java`). It may answer
  with text, a ready NLU intent, a Chinese command to try, or launch a user‑installed app (e.g. a
  Yandex Navigator route). With `OFFLINE_ONLY=true` or no network the assistant is fully offline.
- **Wake** — the wake word «нихао» (你好) or the steering‑wheel key. The key plays a short chime and you
  can talk right away; the wake word plays the chime together with the spoken greeting.
...
The `ru2zh` command mapper (`stand/asr-android/src/com/stand/bridge/Ru2Zh.java`, the very file that ships
in the APK) has offline unit tests — no car, no models, just a JDK. 900+ phrase→command cases plus a set
of chatter that must *not* trigger a command:
...
stand/asr-android/           our code + models
  src/com/stand/**           RuBridge (ASR tap, routing, backend), Ru2Zh (RU→ZH command mapper), GigaAsr, TeraTts, WakeChime, ...
...
ru2zh/translate-task/tests/  offline unit tests for Ru2Zh.java (tests.tsv, chatter.txt, run_tests.sh)
```
