...
**Версия:** 1.0.4 · Распространяется бесплатно.
...
Публикация релиза (для мейнтейнера): `./release.sh 1.0.4 ./SpeechAssistant.orig.apk` — собирает,
гоняет тесты, ставит тег `v1.0.4` и загружает APK в релиз через `gh`. Альтернатива —
