...
public final class TeraTts {
    ...
    private static synchronized void speakStreaming(String text) throws InterruptedException {
        ...
        // Size the track to the first sentence, as before. An oversized buffer (20 s) produced a track that
        // never started on a Changan head unit: getPlaybackHeadPosition stayed at 0 and the whole reply was
        // lost. Later sentences go into the same track; write() blocks until room frees, which is exactly the
        // overlap we want because sentence N+1 is synthesized before it is written.
