 ...
 * E07 keeps OEM capture/wake/NLU/UI; the existing SR handoff supplies native SE containers.
 * SeProbe can copy full OEM frames at the SR handoff without changing capture or wake.
 */
public final class RuBridge {
    ...
    static void ensureTtsRegistered() {
        // E07 uses per-player hooks; never edit the OEM registry or wake settings.
    }
    ...
    private static final boolean OFFLINE_ONLY = true;  // E07: author backend disabled at every shared sender.
    ...
    static String chooseQuery(String text) {
        ...
        if (CommandHelp.topic(text) >= 0) return text;
        ...
        if (Ru2Zh.isStopRequest(text.toLowerCase(java.util.Locale.ROOT).replace('ё', 'е'))) return text;
        String fx = fuzzyFix(text);
        if (!fx.equals(text) && ru2zh(fx, wakeZone) != null) return fx;          // fuzzy rescue
        return fuzzyFix(text);
    }

    /** E07 runs ASR and TTS in the main OEM process. No registry/settings changes or backend startup. */
    public static void init(final Context ctx) {
        try {
            if (ctx == null || !isMainProcess()) return;
            appCtx = ctx.getApplicationContext();
            Ru2Zh.onUnsafeBlocked = new Ru2Zh.UnsafeListener() { public void blocked(String zh) {
                Log.w(TAG, "unsafe command blocked: " + zh);
            }};
            com.stand.asr.GigaAsr.init(appCtx);
            com.stand.tts.TeraTts.init(appCtx);
            SeProbe.init(appCtx);
        } catch (Throwable t) { Log.e(TAG, "init; stock remains available", t); }
    }
    ...
    private static final class StockFrame {
        ...
        final int mask;
        StockFrame(byte[] bytes, long receivedAt, int mask) {
            this.bytes = bytes; this.receivedAt = receivedAt; this.mask = mask;
        }
    }

    private static final class RuSession {
        // ponytail: 32 native frames buffer about 512 ms; overflow invalidates the utterance.
        final java.util.concurrent.ArrayBlockingQueue<StockFrame> frames =
                new java.util.concurrent.ArrayBlockingQueue<StockFrame>(32);
        final int direction;
        final int vadRms;
        final long started = android.os.SystemClock.elapsedRealtime();
        final String id = Long.toHexString(started);
        boolean recognizing;
        volatile boolean recording = true;
        volatile boolean cancelled;
        volatile boolean decode = true;
        volatile String captureFailure;
        RuSession(int direction) {
            this.direction = direction; this.vadRms = vadThreshold();
        }
    }

    /** Only receives the existing OEM SR handoff. Own the full container before callback return.
     *  Time/TTS state are sampled at receipt; no audio capture, conversion, settings or I/O run here. */
    public static boolean acceptStockFrame(byte[] frame, int callbackLength) {
        RuSession session = ruSession;
        if (session == null) return false;
        // This lock is held only for queue acceptance/closure, never conversion or inference.
        synchronized (session) {
            try {
                if (!session.recording || session.cancelled) return true;
                int length = frame == null || frame.length < 512 ? 0
                        : (frame[20] & 255) | ((frame[21] & 255) << 8)
                        | ((frame[22] & 255) << 16) | ((frame[23] & 255) << 24);
                if (length < 512 || length > 65536 || length > frame.length
                        || callbackLength <= 0 || callbackLength > frame.length) {
                    session.captureFailure = "invalid-se-frame";
                    session.decode = false; session.recording = false;
                    return true;
                }
                // ISSSeImpl still reports 3584 even for a full 7168-byte native packet.
                long receivedAt = android.os.SystemClock.elapsedRealtime();
                int mask = speechMask();
                if (!session.frames.offer(new StockFrame(java.util.Arrays.copyOf(frame, length), receivedAt, mask))) {
                    session.captureFailure = "se-queue-overflow";
                    session.decode = false; session.recording = false;
                }
            } catch (Throwable ignored) {
                session.captureFailure = "se-feed-error";
                session.decode = false; session.recording = false;
            }
        }
        // Keep the OEM SR suppressed after endpoint, malformed input and cancellation of capture.
        return true;
    }

    private static int vadThreshold() {
        try {
            int value = android.provider.Settings.Global.getInt(appCtx.getContentResolver(),
                    "e07_voice_vad_rms", Utterance.DEFAULT_RMS_THRESHOLD);
            return value >= 100 && value <= 10000 ? value : Utterance.DEFAULT_RMS_THRESHOLD;
        } catch (Throwable ignored) { return Utterance.DEFAULT_RMS_THRESHOLD; }
    }

    /** Diagnostic only: our start hook skips OEM SR, so its phases cannot delimit RU audio.
     *  Local Utterance endpointing uses the received native PCM. */
    public static void onSrPhase(int type, String msg) {
        if (type == 1105 && msg != null) Log.i(TAG, "OEM SR phase (diagnostic): " + msg);
    }

    /** Ownership survives auto endpoint/decode until the OEM explicitly stops or starts again. */
    public static boolean isRuSessionActive() { return ruSession != null; }

    /** Called instead of SrEngineProxy.start. False means execute the original OEM start. */
    public static boolean startSession(int direction) {
        try {
            // stop() invokes OEM end callbacks; keep those outside SESSION_LOCK too.
            com.stand.tts.TeraTts.stop();
            speechMask(); // Prime the cached OEM pure-Java TTS getter outside the capture callback.
            synchronized (SESSION_LOCK) {
                RuSession old = ruSession;
                if (old != null) { old.cancelled = true; old.recording = false; recognizeState(old, false); ruSession = null; }
                if (appCtx == null || !com.stand.asr.GigaAsr.ready()) return false;
                android.media.AudioManager am = (android.media.AudioManager) appCtx.getSystemService(Context.AUDIO_SERVICE);
                if (am != null && am.getMode() != android.media.AudioManager.MODE_NORMAL) return false;
                try {
                    final RuSession session = new RuSession(direction);
                    wakeZone = direction; lastText = ""; ruSession = session;
                    new Thread(new Runnable() { public void run() { recordSession(session); } }, "RuStockAudio").start();
                    Log.i(TAG, "RU session accepted id=" + session.id + " direction=" + direction
                            + " vadRms=" + session.vadRms + " source=stock-se");
                    return true;
                } catch (Throwable t) {
                    ruSession = null;
                    Log.e(TAG, "RU session start failed; use stock", t);
                    return false;
                }
            }
        } catch (Throwable t) { Log.e(TAG, "startSession; use stock", t); return false; }
    }

    /** decode=false cancels even an in-flight decode; only the current uncancelled session may inject. */
    public static void stopSession(boolean decode) {
        synchronized (SESSION_LOCK) {
            RuSession session = ruSession;
            if (session == null) return;
            if (!decode) session.decode = false; // Integrity failures stay latched, even during stop(true).
            session.cancelled = !decode;
            session.recording = false;
            Log.i(TAG, "RU stop id=" + session.id + " decode=" + decode + " recognizing=" + session.recognizing
                    + " caller=" + java.util.Arrays.toString(Thread.currentThread().getStackTrace()));
            if (!decode) { recognizeState(session, false); ruSession = null; }
        }
    }

    private static void recognizeState(RuSession session, boolean active) {
        if (session.recognizing == active) return;
        try {
            Class.forName("com.incall.apps.speechassistant.common.VoiceStateCache")
                .getMethod(active ? "startRecognize" : "stopRecognize", int.class).invoke(null, session.direction);
            session.recognizing = active;
            Log.i(TAG, "RU recognize-state id=" + session.id + " active=" + active);
        } catch (Throwable t) { Log.e(TAG, "RU recognize-state", t); }
    }

    /** Restart the OEM timer after our work; refreshTimer cannot revive an already expired timer. */
    private static void finishRecognize(final RuSession session) {
        synchronized (SESSION_LOCK) {
            if (ruSession != session) return;
            recognizeState(session, false);
        }
        new android.os.Handler(android.os.Looper.getMainLooper()).post(new Runnable() { public void run() {
            synchronized (SESSION_LOCK) {
                if (ruSession != session || session.cancelled) return;
                try {
                    Class<?> c = Class.forName("com.incall.apps.speechassistant.controller.BusinessController");
                    c.getMethod("startTimer").invoke(c.getMethod("getInstance").invoke(null));
                    Log.i(TAG, "RU timer resumed id=" + session.id);
                } catch (Throwable t) { Log.e(TAG, "resume OEM timer", t); }
            }
        }});
    }

    private static volatile java.lang.reflect.Method stockTtsState;
    /** Bit 1: our TTS, bit 2: OEM TTS, bit 4: cannot establish OEM state. */
    private static int speechMask() {
        int mask = com.stand.tts.TeraTts.isSpeaking() ? 1 : 0;
        try {
            java.lang.reflect.Method m = stockTtsState;
            if (m == null) {
                m = Class.forName("com.incall.apps.speechassistant.common.VoiceStateCache").getMethod("getTtsState");
                stockTtsState = m;
            }
            if (Boolean.TRUE.equals(m.invoke(null))) mask |= 2;
        } catch (Throwable t) { mask |= 4; }
        return mask;
    }

    private static void recordSession(RuSession session) {
        java.io.ByteArrayOutputStream pcm = new java.io.ByteArrayOutputStream(16000 * 2 * 8);
        long now = android.os.SystemClock.elapsedRealtime();
        final long captureStarted = session.started;
        // OEM SR is bypassed during RU ownership; do not wait for its speech_end.
        Utterance endpoint = new Utterance(captureStarted, session.vadRms);
        Utterance.Levels raw = new Utterance.Levels(), postTts = new Utterance.Levels();
        byte[] chunk = new byte[0];
        long quietAfterTts = 0, lastLog = now, lastFrameAt = 0, readBytes = 0, dropBytes = 0;
        int reads = 0, emptyReads = 0, lastMask = -1;
        String reason = "explicit-stop";
        try {
            while (session.recording && !session.cancelled) {
                StockFrame frame = session.frames.poll(100, java.util.concurrent.TimeUnit.MILLISECONDS);
                now = android.os.SystemClock.elapsedRealtime();
                if (!session.recording || session.cancelled) {
                    if (frame != null && session.decode && !session.cancelled) {
                        session.captureFailure = "se-stop-with-pending-frames";
                ...
                if (frame == null) {
                    emptyReads++;
                    // OEM produces a frame every 16 ms, including acoustic silence.
                    if (lastFrameAt != 0 && now - lastFrameAt > 500) {
                        reason = "se-stream-stalled"; session.decode = false; break;
                    }
                    if (lastFrameAt == 0 && now - captureStarted >= Utterance.NO_SPEECH_MS) {
                        reason = "no-speech-timeout"; break;
                    }
                } else {
                    if (lastFrameAt != 0 && frame.receivedAt - lastFrameAt > 500) {
                        reason = "se-stream-stalled"; session.decode = false; break;
                    }
                    // Existing validated OEM extractor; stream choice awaits a real voice recording.
                    chunk = SePcm.extract(frame.bytes, frame.bytes.length);
                    n = chunk.length;
                    lastFrameAt = frame.receivedAt;
                }
                readBytes += n; raw.add(chunk, n);
                long audioNow = frame == null ? now : frame.receivedAt;
                int mask = frame == null ? speechMask() : frame.mask;
                if (mask != 0) quietAfterTts = audioNow + 350;
                boolean muted = mask != 0 || audioNow < quietAfterTts;
                if (mask != lastMask) {
                    Log.i(TAG, "RU TTS gate id=" + session.id + " mask=" + mask + " tail=" + (audioNow < quietAfterTts));
                    lastMask = mask;
                }
                if (muted) {
                    dropBytes += n;
                    pcm.reset(); endpoint = new Utterance(audioNow, session.vadRms);
                } else {
                    if (n > 0) { pcm.write(chunk, 0, n); postTts.add(chunk, n); endpoint.accept(chunk, n, audioNow); }
                    if (endpoint.hasSpeech() && !session.recognizing) {
                        synchronized (SESSION_LOCK) {
                            if (ruSession == session && !session.cancelled) recognizeState(session, true);
                        }
                    }
                    if (frame != null && endpoint.finished(audioNow)) {
                        reason = now - captureStarted >= Utterance.MAX_MS ? "capture-limit"
                                : endpoint.hasSpeech() ? "silence-endpoint" : "no-speech-timeout";
                        break;
                    }
                }
                if (now - lastLog >= 1000) {
                    Log.i(TAG, "RU stock audio id=" + session.id + " elapsed=" + (now - captureStarted)
                            + " readBytes=" + readBytes + " reads=" + reads + " emptyReads=" + emptyReads
                            + " rawRms=" + raw.rms() + " rawPeak=" + raw.peak
                            + " postTtsRms=" + postTts.rms() + " postTtsPeak=" + postTts.peak
                            + " keptBytes=" + pcm.size() + " dropBytes=" + dropBytes + " mask=" + mask
                            + " muted=" + muted + " speech=" + endpoint.hasSpeech());
                    raw.reset(); postTts.reset(); lastLog = now;
                }
                if (now - captureStarted >= Utterance.MAX_MS || pcm.size() >= 16000 * 2 * 15) { reason = "capture-limit"; break; }
            }
        } catch (Throwable t) { reason = "capture-error"; session.decode = false; Log.e(TAG, "recordSession", t); }
        finally {
            synchronized (session) {
                session.recording = false;
                if ("explicit-stop".equals(reason) && session.decode && !session.frames.isEmpty()) {
                    session.captureFailure = "se-stop-with-pending-frames";
                    session.decode = false;
                }
                session.frames.clear();
                if (session.captureFailure != null) reason = session.captureFailure;
            }
            Log.i(TAG, "RU capture end id=" + session.id + " reason=" + reason + " elapsed="
                    + (android.os.SystemClock.elapsedRealtime() - captureStarted) + " cancelled=" + session.cancelled
                    + " decode=" + session.decode + " speech=" + endpoint.hasSpeech() + " readBytes=" + readBytes
                    + " keptBytes=" + pcm.size() + " dropBytes=" + dropBytes
                    + " rawRms=" + raw.rms() + " rawPeak=" + raw.peak
                    + " postTtsRms=" + postTts.rms() + " postTtsPeak=" + postTts.peak);
        }
        try {
            if (session.cancelled || !session.decode) return;
            synchronized (SESSION_LOCK) {
                if (ruSession != session || session.cancelled || !session.decode) return;
            }
            // Keep VoiceStateCache's recognition bit through native inference: SleepTimer consults it.
            Log.i(TAG, "RU decode begin id=" + session.id + " speech=" + endpoint.hasSpeech() + " bytes=" + pcm.size());
            String text = endpoint.hasSpeech() ? com.stand.asr.GigaAsr.finish(pcm.toByteArray()) : "";
            String query;
            synchronized (SESSION_LOCK) {
                if (ruSession != session || session.cancelled || !session.decode) {
                    Log.i(TAG, "RU decode discarded id=" + session.id + " cancelled=" + session.cancelled); return;
                }
                Log.i(TAG, "RU decode result id=" + session.id + " text=" + text);
                query = chooseQuery(text);
                if (query == null || query.isEmpty()) { Log.i(TAG, "RU session ended without speech id=" + session.id); return; }
                lastText = query;
                showOnScreen(query, TYPE_NLP);
            }
            handlePhraseZh(query, session);
        } catch (Throwable t) { Log.e(TAG, "decode/inject", t); }
        finally { finishRecognize(session); }
    }
    ...
    private static void speak(String text) {
        try {
            Class<?> tc = Class.forName("com.incall.apps.speechassistant.tts.GlobalTtsClient");
    ...
    public static String ttsTextToRu(String text) {
        ...
        if (hasCyrillic(s)) {                                      // overlay Russian (maybe + stray CJK)
            ...
            // An unknown complete OEM reply may be a refusal. Do not reconstruct it from component words.
        } else {
    ...
    /** Translate known OEM replies; preserve unknown original text for the stock fallback. */
    ...
    public static boolean onTtsText(String text, final Object listener) {
        return onTtsText(text, listener, listener);
    }

    /** Per-player ownership: other OEM players must retain their own isPlay/stop semantics. */
    public static boolean onTtsText(String text, final Object listener, final Object player) {
        try {
            if (!com.stand.tts.TeraTts.ready() || text == null || text.trim().isEmpty()) return false;
            if (listener != null && Class.forName("com.incall.apps.speechassistant.tts.IPlayerDataListener").isInstance(listener)) return false;
            final String ru = hasCJK(text) ? zh2ru(text) : (hasCyrillic(text) ? text : null);
            if (ru == null || ru.trim().isEmpty()) {
                // Без этой строки непереведённый ответ уходит в штатную озвучку молча и найти его
                // в логе невозможно: так искали китайский ответ про подогрев 13.09.2026.
                if (hasCJK(text)) Log.i(TAG, "no RU for OEM reply, stock TTS speaks it: [" + text + "]");
                return false;
            }
            final boolean stockFocus = Class.forName("com.incall.apps.speechassistant.tts.TtsPlayer").isInstance(player);
            final int type = playerInt(player, "mType", 100);
            int usage = playerInt(player, stockFocus ? "mUsage" : "mAudioUsage", 18);
            return com.stand.tts.TeraTts.speakOwned(ru, player, usage, stockFocus, new com.stand.tts.TeraTts.PlaybackListener() {
                public void begin() {
                    ttsState(type, true);
                    callListener(listener, "onPlayBegin", String.class, ru);
                }
                public void progress() { ttsTimer(type); }
                public void end(int status) {
                    // TtsSortStrategy pre-acquired this TtsPlayer's focus before its start hook.
                    // TtsPlayer2 has no such pre-acquisition; Tera owns and releases its focus instead.
                    if (stockFocus) invokePlayer(player, "abandonFocus");
                    ttsState(type, false);
                    if (status != 0) {
                        try { Class.forName("com.incall.apps.speechassistant.tts.TtsSortStrategy")
                            .getMethod("clearLevel", int.class).invoke(null, type); }
                        catch (Throwable t) { Log.e(TAG, "clearLevel", t); }
                    }
                    if (status == 0) callListener(listener, "onPlayComplete", null, null);
                    else if (status == 1) callListener(listener, "onPlayInterrupt", null, null);
                    else callListener(listener, "onError", int.class, Integer.valueOf(-1));
                }
            });
        } catch (Throwable t) { Log.e(TAG, "onTtsText; use stock", t); return false; }
    }

    public static boolean isSpeaking(Object player) { return com.stand.tts.TeraTts.isSpeaking(player); }
    public static boolean stopSpeech(Object player) { return com.stand.tts.TeraTts.stop(player); }
    public static boolean isSpeaking() { return com.stand.tts.TeraTts.isSpeaking(); }
    public static void stopSpeech() { com.stand.tts.TeraTts.stop(); }

    private static int playerInt(Object player, String name, int fallback) {
        if (player == null) return fallback;
        for (Class<?> c = player.getClass(); c != null; c = c.getSuperclass()) {
            try { java.lang.reflect.Field f = c.getDeclaredField(name); f.setAccessible(true); return f.getInt(player); }
            catch (NoSuchFieldException missing) {} catch (Throwable t) { Log.e(TAG, "player field " + name, t); break; }
        }
        return fallback;
    }
    private static void invokePlayer(Object player, String name) {
        if (player == null) return;
        for (Class<?> c = player.getClass(); c != null; c = c.getSuperclass()) {
            try { java.lang.reflect.Method m = c.getDeclaredMethod(name); m.setAccessible(true); m.invoke(player); return; }
            catch (NoSuchMethodException missing) {} catch (Throwable t) { Log.e(TAG, "player method " + name, t); return; }
        }
    }
    private static void ttsState(int type, boolean playing) {
        try {
            Class.forName("com.incall.apps.speechassistant.common.VoiceStateCache")
                .getMethod(playing ? "startTts" : "stopTts", int.class).invoke(null, type);
        } catch (Throwable t) { Log.e(TAG, "TTS state", t); }
        ttsTimer(type);
    }
    private static void ttsTimer(int type) {
        try {
            Class.forName("com.incall.apps.speechassistant.timer.TimerRefresh")
                .getMethod("refreshTimer", int.class, int.class).invoke(null, type, -1);
        } catch (Throwable t) { Log.e(TAG, "TTS timer", t); }
    }

    private static void callListener(Object l, String method, Class<?> sig, Object arg) {
        if (l == null) return;
        try {
            java.lang.reflect.Method m = sig == null ? l.getClass().getMethod(method) : l.getClass().getMethod(method, sig);
            m.setAccessible(true); // OEM listeners are often anonymous, non-public classes.
            if (sig == null) m.invoke(l); else m.invoke(l, arg);
    ...
    static String zh2ru(String zh) {
        ...
        String s = zh.trim().replaceAll("\\[(?:[a-zA-Z]\\d+|=[^\\]]+)\\]", ""); // OEM prosody/pronunciation tags
        s = s.replaceAll("^[，、,。~\\s]+", "").replaceAll("[，。.!！~\\s]+$", "");
        // Preserve a seat as part of a structured status; only greeting prefixes are stripped below.
        String direct = zh2ruCore(s);
    ...
    static String zh2ruCore(String s) {
        String oem = oemReply(s);
        if (oem != null) return oem;
        // Observed E07 REMAINING_POWER reply. TeraTts.ttsNormalize expands % and км with agreement.
        java.util.regex.Matcher battery = java.util.regex.Pattern.compile(
                "剩余电量\\s*(100|[0-9]{1,2})\\s*[%％]\\s*[,，]\\s*可行驶\\s*([0-9]{1,4})\\s*公里[。.!！]?\\s*").matcher(s);
        if (battery.matches()) return "Заряд батареи: " + battery.group(1) + "%. Запас хода: " + battery.group(2) + " км";
        switch (s) {
    ...
    /** Exact E07 resource templates: CaSpeechAdapter res/values/strings.xml aircontrol_* and
     * carcontrol_chair_*, window_*, trunk_*, mot_window_*. A mentioned component never implies success. */
    static String oemReply(String text) {
        String s = text.replace('，', ',');
        // На машине с локалью en-US штатный DM отдаёт утвердительный префикс латиницей:
        // answer='OK,副驾座椅[w0]加热已关闭' вместо 好的. Подменяется он вне ресурсов, поэтому
        // приводим к китайскому виду один раз здесь — иначе мимо проходят все ветки с 好的.
        s = s.replaceFirst("^(?:OK|Ok|ok|ОК|Ок)[,，]\\s*", "好的,");
        switch (s) {
            case "语音执行失败了,建议手动再尝试一下": return "Не удалось выполнить голосовую команду. Попробуйте вручную";
            case "抱歉,该功能暂不支持呢": case "抱歉,该功能暂不支持": case "当前不支持": return "Эта функция сейчас не поддерживается";
            case "抱歉,该功能暂不支持车外语音操作": return "Эта функция не поддерживает голосовое управление снаружи машины";
            case "车门锁不支持语音控制": return "Блокировка дверей голосом не поддерживается";
            case "暂不支持该挡位": return "Этот уровень не поддерживается";
            case "超出可控范围咯": return "Значение вне допустимого диапазона";
            case "该功能尚未开启": return "Эта функция ещё не включена";
            case "好的,空调已打开": return "Климат включён";
            case "好的,空调已关闭": return "Климат выключен";
            case "空调已经打开了": return "Климат уже включён";
            case "空调已经关闭了": return "Климат уже выключен";
            case "当前空调未开启": case "空调还没有打开": return "Климат ещё не включён";
            case "空调自动模式已开启": return "Автоматический режим климата включён";
            case "自动模式无法直接关闭": return "Автоматический режим нельзя выключить напрямую";
            case "自动模式并未开启": return "Автоматический режим ещё не включён";
            case "空调出风模式无法关闭,仅支持切换": return "Режим обдува можно только переключить";
            case "空调已打开,没有你说的出风模式": return "Климат включён, но такого режима обдува нет";
            case "你说的出风模式没有呢": return "Такого режима обдува нет";
            case "语音不支持这种方式控制空调温度哦": return "Такое изменение температуры голосом не поддерживается";
            case "语音不支持这种方式控制空调风量哦": return "Такое изменение обдува голосом не поддерживается";
            case "抱歉,后排不支持单独控制风速": return "Отдельное управление обдувом сзади не поддерживается";
            case "各个区域温度不同,请分开调节哦": return "Температуры в зонах различаются. Настройте зоны отдельно";
            case "已调到最高温度": return "Установлена максимальная температура";
            case "已调到最低温度": return "Установлена минимальная температура";
            case "风速已经是最大挡了": return "Обдув уже на максимуме";
            case "风速已经是最小挡了": return "Обдув уже на минимуме";
            case "已增加到最大挡了": return "Обдув установлен на максимум";
            case "已降低到最小挡了": return "Обдув установлен на минимум";
            case "内循环已开启": return "Рециркуляция включена";
            case "外循环已开启": return "Забор наружного воздуха включён";
            case "温区同步已关闭": return "Синхронизация температуры выключена";
            case "温区同步已开启,全车温区将统一": return "Синхронизация температуры включена для всех зон";
            case "好的,后除霜已打开": return "Обогрев заднего стекла включён";
            case "好的,后除霜已关闭": return "Обогрев заднего стекла выключен";
            case "后除霜已经打开了": return "Обогрев заднего стекла уже включён";
            case "后除霜已经关闭了": return "Обогрев заднего стекла уже выключен";
            case "前除霜未开启": return "Обдув лобового стекла ещё не включён";
            case "主驾请驻车后再调节座椅": return "Перед регулировкой сиденья водителя остановите машину";
            case "后排暂不支持调节哦": return "Регулировка задних сидений не поддерживается";
            case "座椅调节已锁定": return "Регулировка сидений заблокирована";
            case "座椅位置校准中,请稍后再试一下": return "Идёт калибровка сиденья. Попробуйте позже";
            case "座椅已折叠,请恢复座椅后再试一下": return "Сиденье сложено. Сначала разложите его";
            case "请先确保后排无人或物品后再试一下": return "Сначала убедитесь, что сзади нет людей и вещей";
            case "好的,后方的乘客请注意安全": return "Хорошо. Пассажиры сзади, будьте осторожны";
            case "车窗锁已打开,仅主驾可调节车窗": return "Окна заблокированы. Управлять ими может только водитель";
            case "好的,车窗已解锁": return "Окна разблокированы";
            case "好的,车窗已上锁": return "Окна заблокированы";
            case "正在打开,请小心": case "好的,正在打开,请注意安全": return "Открывается. Будьте осторожны";
            case "正在关闭,请小心": case "好的,正在关闭,请注意安全": return "Закрывается. Будьте осторожны";
            case "已经打开了": return "Уже открыто";
            case "已经关闭了": return "Уже закрыто";
            case "前备箱已解锁,请到车外手动闭锁": return "Передний багажник разблокирован. Закройте его вручную снаружи";
            case "前备箱没有电动控制功能,请手动关闭": return "У переднего багажника нет электропривода. Закройте его вручную";
            case "好的,前备箱已解锁,请手动开启": case "前备箱已经解锁了,请手动开启": return "Передний багажник разблокирован. Откройте его вручную";
            case "前备箱防夹/防撞触发,请检查": return "Сработала защита переднего багажника. Проверьте препятствия";
            case "好的,后备门玻璃即将打开,请小心": return "Стекло багажника открывается. Будьте осторожны";
            case "好的,后备门玻璃即将关闭,请小心": return "Стекло багажника закрывается. Будьте осторожны";
            case "行车时不支持打开后备窗": return "Открытие стекла багажника во время движения запрещено";
            case "天窗正开着,此时后背窗无法控制呢": return "Сначала закройте люк, затем регулируйте стекло багажника";
            case "后背门正开着,此时后背窗无法控制呢": return "Сначала закройте задний борт, затем регулируйте стекло багажника";
            case "请先关闭隔断玻璃锁后再试一下": return "Сначала снимите блокировку стекла перегородки";
            case "请关闭后隔断后再试一下": return "Сначала закройте заднюю перегородку";
            case "好的,隔断玻璃锁已关闭": return "Стекло перегородки разблокировано";
            case "好的,隔断玻璃锁已打开": return "Стекло перегородки заблокировано";
            case "隔断玻璃锁已经关闭了": return "Стекло перегородки уже разблокировано";
            case "隔断玻璃锁已经打开了": return "Стекло перегородки уже заблокировано";
            case "不支持这种座椅按摩模式": return "Такой режим массажа не поддерживается";
            case "不支持这样调节座椅位置呢": return "Так менять положение сиденья нельзя";
            case "好的,正在折叠座椅,请注意安全": return "Складываю сиденье. Будьте осторожны";
            case "好的,正在展开座椅,请注意安全": return "Раскладываю сиденье. Будьте осторожны";
            case "靠背已经折叠了": return "Спинка уже сложена";
            case "靠背已经展开了": return "Спинка уже разложена";
            case "好的,靠背已暂停运动": return "Спинка остановлена";
            case "座椅按摩已经是强力挡啦": return "Массаж: уже самый сильный";
            case "座椅按摩已经是轻柔挡啦": return "Массаж: уже самый мягкий";
        }
        java.util.regex.Matcher m = java.util.regex.Pattern.compile("已调到([0-9]{1,2}(?:\\.[0-9])?)度").matcher(s);
        if (m.matches()) return "Температура установлена: " + m.group(1).replace('.', ',');
        m = java.util.regex.Pattern.compile("已调(高|低)([0-9]{1,2}(?:\\.[0-9])?)度").matcher(s);
        if (m.matches()) return "Температура " + (m.group(1).equals("高") ? "повышена" : "понижена") + " на " + m.group(2).replace('.', ',');
        m = java.util.regex.Pattern.compile("已(?:增加|降低)到([0-8])挡了").matcher(s);
        if (m.matches()) return "Обдув установлен на уровень " + m.group(1);
        String position = "(左后排|右后排|后排左|后排右|主驾|副驾|前排|后排|左前|右前|左后|右后|左侧|右侧|所有|全车)?";
        m = java.util.regex.Pattern.compile("(?:好的,)?" + position + "座椅(加热|通风|按摩)(已打开|已关闭|已经开啦|已经关啦|已经打开了|已经关闭了|未开启)").matcher(s);
        if (m.matches()) {
            String function = m.group(2).equals("加热") ? "Подогрев" : m.group(2).equals("通风") ? "Вентиляция" : "Массаж";
            String state = m.group(3);
            boolean off = state.equals("未开启") || state.contains("关");
            String ending = m.group(2).equals("通风") ? "а" : "";
            return function + " сиденья" + oemPosition(m.group(1)) + (state.equals("未开启") ? " ещё не включен" : state.startsWith("已经") ? " уже " + (off ? "выключен" : "включен") : " " + (off ? "выключен" : "включен")) + ending;
        }
        m = java.util.regex.Pattern.compile("抱歉,本车不支持" + position + "座椅(加热|通风|按摩)").matcher(s);
        if (m.matches()) return (m.group(2).equals("加热") ? "Подогрев" : m.group(2).equals("通风") ? "Вентиляция" : "Массаж")
                + " сиденья" + oemPosition(m.group(1)) + " не поддерживается";
        m = java.util.regex.Pattern.compile("好的," + position + "座椅(加热|通风|按摩)已(?:设置为|设为)([1-3])挡").matcher(s);
        if (m.matches()) return (m.group(2).equals("加热") ? "Подогрев" : m.group(2).equals("通风") ? "Вентиляция" : "Массаж")
                + " сиденья" + oemPosition(m.group(1)) + ": уровень " + m.group(3);
        // Пределы уровня. Китайская строка не называет функцию, поэтому формулировка нейтральная.
        m = java.util.regex.Pattern.compile(position + "座椅已经是(最高|最低)挡啦").matcher(s);
        if (m.matches()) return "Сиденье" + oemPosition(m.group(1)) + ": уже "
                + (m.group(2).equals("最高") ? "максимальный" : "минимальный") + " уровень";
        m = java.util.regex.Pattern.compile(position + "座椅通风挡位已经(最大|最小)咯").matcher(s);
        if (m.matches()) return "Вентиляция сиденья" + oemPosition(m.group(1)) + ": уже "
                + (m.group(2).equals("最大") ? "максимальный" : "минимальный") + " уровень";
        m = java.util.regex.Pattern.compile(position + "座椅按摩已经是(强力|轻柔)挡").matcher(s);
        if (m.matches()) return "Массаж сиденья" + oemPosition(m.group(1)) + ": уже "
                + (m.group(2).equals("强力") ? "самый сильный" : "самый мягкий");
        // Массаж: уровень, режим, выключение, «ещё не включён».
        m = java.util.regex.Pattern.compile("好的," + position + "座椅按摩挡位已调(高|低)").matcher(s);
        if (m.matches()) return "Массаж сиденья" + oemPosition(m.group(1)) + ": уровень "
                + (m.group(2).equals("高") ? "повышен" : "понижен");
        m = java.util.regex.Pattern.compile("好的," + position + "座椅按摩模式已切换").matcher(s);
        if (m.matches()) return "Режим массажа сиденья" + oemPosition(m.group(1)) + " переключён";
        m = java.util.regex.Pattern.compile("已关闭" + position + "座椅按摩").matcher(s);
        if (m.matches()) return "Массаж сиденья" + oemPosition(m.group(1)) + " выключен";
        m = java.util.regex.Pattern.compile(position + "座椅按摩还没有打开").matcher(s);
        if (m.matches()) return "Массаж сиденья" + oemPosition(m.group(1)) + " ещё не включён";
        m = java.util.regex.Pattern.compile("暂不支持调节" + position + "座椅").matcher(s);
        if (m.matches()) return "Регулировка сиденья" + oemPosition(m.group(1)) + " не поддерживается";
        // Пять штатных формулировок «настройки сидений различаются» сводятся к одной русской.
        m = java.util.regex.Pattern.compile(position
                + "座椅(?:的|加热|通风的?|按摩强度|按摩模式)?(?:挡位)?不同哟?,\\s*(?:建议单独设置哟|请分别设置吧)").matcher(s);
        if (m.matches()) return "Настройки сидений различаются. Задайте каждое отдельно";
        m = java.util.regex.Pattern.compile(position + "车窗(已经打开了|已经关闭了|已经关闭了呢|已开最大了呢|已经打开到位了)").matcher(s);
        if (m.matches()) return "Окно" + oemPosition(m.group(1)) + (m.group(2).contains("关") ? " уже закрыто" : m.group(2).equals("已经打开到位了") ? " уже в заданном положении" : m.group(2).equals("已开最大了呢") ? " уже полностью открыто" : " уже открыто");
        return null;
    }

    private static String oemPosition(String zh) {
        if (zh == null) return "";
        switch (zh) {
            case "主驾": case "左前": return " водителя";
            case "副驾": case "右前": return " переднего пассажира";
            case "后排左": case "左后": case "左后排": return " сзади слева";
            case "后排右": case "右后": case "右后排": return " сзади справа";
            case "左侧": return " слева";
            case "右侧": return " справа";
            case "前排": return " спереди";
            case "后排": return " сзади";
            case "所有": case "全车": return " во всех зонах";
            default: return "";
        }
    }
    ...
    //   static fields com.incall.apps.speechassistant.common.ComVar.{sVin,sTuid,sCarModelItems}.
    ...
    private static String comVar(String field) {
        try {
            Class<?> c = Class.forName("com.incall.apps.speechassistant.common.ComVar");
    ...
    /** confident=true selects the OEM confident-local result category for mapped commands.
     *  Stock E07 can still wait for its cloud/arbitration timeout; this argument alone does not bypass it. */
    public static void injectZh(final String zh, final boolean confident) {
        injectZh(zh, confident, ruSession);
    }

    private static void injectZh(final String zh, final boolean confident, final RuSession session) {
        if (!isMainProcess() || zh == null || zh.isEmpty()) return;
        synchronized (SESSION_LOCK) {
            if (session == null || session != ruSession || session.cancelled) return;
        }
        E07Trunk trunk = E07Trunk.parse(zh);
        if (trunk != null) {
            String reply;
            try {
                Object sdk = trunk.refusal == null
                        ? Class.forName("com.incall.apps.casdkmanager.CaSdkManager").getMethod("getInstance").invoke(null)
                        : null;
                reply = trunk.execute(sdk, SESSION_LOCK, () -> ruSession == session && !session.cancelled);
            } catch (Throwable t) {
                Log.e(TAG, "E07 trunk SDK command failed: " + zh, t);
                reply = "Не удалось передать команду багажнику";
            }
            synchronized (SESSION_LOCK) {
                if (reply == null || ruSession != session || session.cancelled) return;
                Log.i(TAG, "E07 trunk: " + zh + " -> " + reply);
                showOnScreen(reply, TYPE_FEEDBACK); speak(reply);
            }
            return;
        }
        synchronized (SESSION_LOCK) {
            if (session != ruSession || session.cancelled) return;
            try {
                Class<?> nmC = Class.forName("com.incall.apps.speechassistant.nlu.NluManager");
                Object nm = nmC.getMethod("getInstance").invoke(null);
                String reqId = "standzh-" + java.util.UUID.randomUUID().toString();
                nmC.getMethod("updateSidTimestamp", String.class).invoke(nm, reqId);
                nmC.getMethod("onFinalAsrResult", String.class, int.class, String.class, boolean.class)
                   .invoke(nm, reqId, session.direction, zh, confident);
                Log.i(TAG, "injectZh direction=" + session.direction + " reqId=" + reqId + ": " + zh);
    ...
    public static void cloudAsk(final String zh) {
        ...
        new Thread(new Runnable() { public void run() {
            try {
                String vin = refStaticStr("com.incall.apps.speechassistant.common.ComVar", "sVin");
                String tuid = refStaticStr("com.incall.apps.speechassistant.common.ComVar", "sTuid");
                String carModel = refStaticStr("com.incall.apps.speechassistant.common.ComVar", "sCarModelItems");
                ...
                Class<?> cfg = Class.forName("com.incall.apps.speechassistant.util.CommonConfig");
                String url = (String) cfg.getMethod("getDialogUrl").invoke(null);

                Class<?> gw = Class.forName("com.incall.apps.speechassistant.util.GateWayUtils");
    ...
    private static void forceWakeIfAsleep(int zone) {
        try {
            Class<?> c = Class.forName("com.incall.apps.speechassistant.common.VoiceStateCache");
...
// no system-language intent to feed it.
    ...
    public static void handlePhraseZh(String ru) {
        handlePhraseZh(ru, ruSession);
    }

    private static void handlePhraseZh(String ru, RuSession session) {
        if (session == null || session != ruSession || session.cancelled) return;
        if (ru == null || ru.trim().isEmpty()) return;
        if (handleHelp(ru)) return;
        ...
        if (zh != null) {
            ...
            Log.i(TAG, "ru2zh: [" + ru + "] -> " + zh); injectZh(zh, true, session); return;
        }
        ...
        if (OFFLINE_ONLY) { Log.i(TAG, "ru2zh: unrecognized (offline): " + ru); String reason = Ru2Zh.limitation(ru); if (reason == null) reason = "Не поняла команду"; showOnScreen(reason, TYPE_FEEDBACK); speak(reason); }
        else { Log.i(TAG, "ru2zh: -> backend chat: " + ru); sendToCloud(ru); }
    }

    private static boolean handleHelp(String query) {
        int topic = CommandHelp.topic(query);
        if (topic < 0) return false;
        Log.i(TAG, "RU local help topic=" + topic);
        showOnScreen(CommandHelp.screen(topic), TYPE_FEEDBACK);
        speak(CommandHelp.spoken(topic));
        return true;
    }
