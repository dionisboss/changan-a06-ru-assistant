...
final class Ru2Zh {
    ...
    /** Release enables the dedicated trunk handler; the broader ADAS/door group stays gated.
     *  Both switches are mutable only so regression tests can exercise explicit profiles. */
    static boolean ALLOW_UNSAFE = false;
    static boolean ALLOW_TRUNK = true;
    ...
    private static final java.util.regex.Pattern NEGATION_WORD = word("не");
    private static final java.util.regex.Pattern AND_WORD = word("и");
    ...
    private static int percentOf(String s) {
        ...
        if (s.contains("процент") || s.contains("%")) {
            int n = numIn(s);
            if (n >= 1 && n <= 100 || n == 0 && trunkRatioTarget(trunkPart(s))) return n;
        }
    ...
    static boolean isGuarded(String s) {
        // Fail closed on ANY standalone «не», including verbs not known to the mapper.
        // N-best arbitration shares this guard, so an alternative cannot drop the negation.
        if (rejectInput(s)) return true;
    ...
    static String ru2zh(String t, int speakerDir) {
        ...
        String s = t.toLowerCase(java.util.Locale.ROOT).replace('ё', 'е');
        // Before even the vehicle/status exceptions: «не показывай пробег» stays inactive.
        if (rejectInput(s)) return null;
        ...
        String zx = zhZone(zoneOf(s));
        if ("后排右".equals(zx)) zx = "右后"; // E07 otherwise collapses this climate zone to REAR.
        if (s.contains("продрог весь") || s.contains("замерз весь")) zx = "";      // EXPLICIT zone only, "" if not named (climate: canonical 把温度调到24度)
        String grade = gradeOf(s);
        int n = numIn(s);
        if (n < 0) n = levelWordOf(s);   // «обдув на пятёрку»
        String r = translate(s, off, z, zx, grade, n, speakerDir);
        if (BLOCKED.equals(r) || r == null) return null;
        // Domain rules often pick a number/mode before the off verb. Do not turn
        // «выключи климат авто» or «закрой окно на 30%» into an enabling command.
        if (isStopRequest(s) && !isStoppingCommand(s, r)) return null;
        return r;
    }

    /** Shared by raw mapping and ASR rescue: rejected input must not be reinterpreted by fuzzyFix. */
    private static boolean rejectInput(String s) {
        if (hasWord(s, NEGATION_WORD)) return true;
        if (limitation(s) != null) return true;
        // No smart-home backend in the E07 profile. Never route a home curtain into the car.
        if (s.contains("дома") || s.contains("домашн") || s.contains("квартир")) return true;
        int n = numIn(s);
        if (n < 0) n = levelWordOf(s);
        boolean negative = s.contains("минус") || java.util.regex.Pattern.compile("-\\s*\\d").matcher(s).find();
        if ((s.contains("процент") || s.contains("%"))
            && (n < 0 || n == 0 && !trunkRatioTarget(trunkPart(s)) || n > 100 || negative)) return true;
        // Keep conjunctions inside one airflow direction or a half-degree value only.
        String clauses = s.replaceAll("(?<![а-я])(?:лицо|ноги|тело|грудь|стекло)\\s+и\\s+(?:в\\s+)?(?:лицо|ноги|тело|грудь|стекло)(?![а-я])", " ");
        if (s.contains("температур") || s.contains("градус")) {
            clauses = clauses.replaceAll("(?<![а-я])и\\s+(?:пять(?:\\s+десятых)?|половиной)(?![а-я])", " ");
        }
        if (hasWord(clauses, AND_WORD)) return true;
        boolean phone = s.contains("звон") || s.contains("вызов") || s.contains("трубк");
        if (!phone && ((s.contains("ответь") && !s.trim().equals("ответь"))
            || (s.contains("сбрось") && !s.trim().equals("сбрось")))) return true;
        // A profile number must not silently become "save the current seat position".
        boolean seatMemory = (s.contains("сиден") || s.contains("кресл"))
            && (s.contains("сохрани") || s.contains("запомни") || s.contains("памят") || s.contains("профил"));
        if (seatMemory && (n >= 0 || ordinalIn(s) > 0)) return true;
        boolean cabinTemp = (s.contains("температур") || s.contains("градус") || s.contains("теплее")
            || s.contains("холодн") || s.contains("климат") || s.contains("кондиц") || s.contains("кондей"))
        ...
        String temperature = cabinTemp ? tempValueOf(s) : null;
        if (temperature != null) {
            double value = Double.parseDouble(temperature);
            boolean relative = s.contains("на ") && (gradeOf(s).equals("PLUS") || gradeOf(s).equals("MINUS")
                || plusWordOf(s) || minusWordOf(s));
            // Gen1 ordinary setTemperature excludes <=17.5 / >=32.5; MIN/MAX use OEM sentinels.
            if (negative || (!relative && (value < 18 || value > 32 || value * 2 != Math.rint(value * 2)))) return true;
            if (relative && (value < 0.5 || value > 8 || value * 2 != Math.rint(value * 2))) return true;
        }
        if (n >= 0 && (s.contains("громкост") || s.contains("громче") || s.contains("тише"))
            && (negative || n > 40)) return true;
        if ((s.contains("процент") || s.contains("%"))
            && (s.contains("экран") || s.contains("дисплей") || s.contains("hud") || s.contains("проекц"))
            && (s.contains("высот") || s.contains("угол") || s.contains("наклон") || s.contains("выше") || s.contains("ниже"))) return true;
        if (n < 0) return false;
        // These groups are enabled on E07: invalid numbers must not fall through to ON/ADD.
        if (s.contains("круиз") && (n < 30 || n > 150 || negative)) return true;
        if (s.contains("дистанц") && (n < 1 || n > 4 || negative)) return true;
        boolean seat = s.contains("сиден") || s.contains("кресл") || s.contains("пердак")
            || s.contains("жоп") || s.contains("булк") || s.contains("пятую точку");
        boolean heat = s.contains("подогре") || s.contains("обогре") || s.contains("грей")
            || s.contains("греть") || s.contains("тепл") || s.contains("попогрей");
        boolean bareHeat = (s.contains("подогре") || s.contains("попогрей") || s.contains("жопогрей"))
            && !s.contains("стек") && !s.contains("лоб") && !s.contains("зеркал")
            && !s.contains("батаре") && !s.contains("аккум") && !s.contains("двигател")
            && !s.contains("холодильник") && !s.contains("морозилк");
        int max = -1;
        if (s.contains("ароматизат") || s.contains("аромат") || s.contains("парфюм")) max = 3;
        else if (s.contains("массаж") || s.contains("массир") || bareHeat
            || (s.contains("рул") && heat)
            || (seat && (heat || s.contains("вентил") || s.contains("продув") || s.contains("обдув")))) max = 3;
        else if (s.contains("дворник") || s.contains("щетк") || s.contains("стеклоочистит")) max = 4;
        else if (!seat && !s.contains("стекл") && !s.contains("лобов")
            && (s.contains("обдув") || s.contains("вентилятор") || s.contains("дуй"))) max = 8;
        return max > 0 && (n < 1 || n > max || negative);
    }

    /** Only observed E07 failures/lost parameters; never substitute another vehicle action. */
    static String limitation(String text) {
        if (text == null) return null;
        String s = text.toLowerCase(java.util.Locale.ROOT).replace('ё', 'е');
        int n = numIn(s);
        boolean volume = s.contains("громк") || s.contains("тише");
        if (s.contains("радио") && n >= 0 && !volume)
            return "Выбор частоты радио пока не поддерживается";
        String trunk = trunkPart(s);
        if (trunk != null && (s.contains("подним") || s.contains("опуст"))
                && (!(trunk.equals("后备窗") || trunk.equals("后隔断玻璃") || trunk.equals("地门"))
                    || percentOf(s) >= 0))
            return "Уточните действие: открой, закрой или останови конкретную часть багажника";
        if (trunk != null && (n >= 0 || percentOf(s) > 0)) {
            if (percentOf(s) < 0 || java.util.regex.Pattern.compile("\\d[.,]\\d").matcher(s).find())
                return "Укажите положение багажника целым числом процентов";
            if (trunkPause(s) || isStopRequest(s))
                return "Скажите отдельно: останови, закрой или открой на нужный процент";
            if (!trunkRatioTarget(trunk))
                return "Положение этой створки в процентах пока не поддерживается";
        }
        if (s.contains("сколько бензина") || s.contains("остаток топлива"))
            return "Запрос топлива пока не поддерживается";
        if ((s.contains("руля") || s.contains("руль")) && (s.contains("подогре") || s.contains("обогре")) && n >= 0)
            return "Обогрев руля можно включить или выключить. Выбор уровня пока не поддерживается";
        if ((s.contains("сиден") || s.contains("кресл"))
            && (s.contains("сохрани") || s.contains("запомни") || s.contains("памят") || s.contains("профил"))
            && (n >= 0 || ordinalIn(s) > 0))
            return "Номер профиля сиденья пока не поддерживается";
        return null;
    }

    static boolean isStopRequest(String s) {
        // «опусти окно» intentionally opens it; that movement verb is not a stop request.
        return s.contains("выключ") || s.contains("отключ") || s.contains("выруб")
            || s.contains("выкл ") || s.contains("закр") || s.contains("останов")
            || hasWord(s, STOP_WORD) || offWordOf(s);
    }

    private static boolean isStoppingCommand(String s, String zh) {
        if (zh.startsWith("关闭") || zh.startsWith("取消") || zh.startsWith("停止")
            || zh.startsWith("退出") || zh.startsWith("收起") || zh.startsWith("折叠")
            || zh.equals("静音") || zh.startsWith("暂停") || zh.equals("退下") || zh.equals("挂断")
            || zh.equals("拒接") || zh.equals("不用了") || zh.equals("息屏")
            || zh.equals("一键全关") || zh.equals("全部关闭") || zh.equals("锁车")) return true;
        // Turning recirculation off intentionally opens the outside-air intake.
        if (zh.equals("打开外循环") && (s.contains("рециркул") || s.contains("циркуляц")
            || (s.contains("забор") && s.contains("воздух")))) return true;
        // «закрывай окна в дождь / закрой окна на охране» enables an auto-close feature.
        // An explicit disable verb must never take this exception.
        boolean explicitOff = s.contains("выключ") || s.contains("отключ") || s.contains("выруб")
            || s.contains("выкл ") || s.contains("останов") || offWordOf(s);
        return !explicitOff && (zh.equals("打开雨天自动关窗") || zh.equals("打开锁车关窗") || zh.equals("打开离车自动闭锁"));
    }
    ...
    private static String translate(String s, boolean off, String z, String zx, String grade, int n, int speakerDir) {
        ...
        if ((r = zhTrunk(s, off)) != null) return r;
    ...
    private static String zhSeats(String s, boolean off, String z, String grade, int n) {
        ...
        if (seat || s.contains("массаж") || s.contains("помассир") || s.contains("массир")
            ...
            if (s.contains("подогре") || s.contains("обогре") || s.contains("грей") || s.contains("греть") || s.contains("тепл")) {
                ...
                if (grade.equals("MIN")) return z + "座椅加热1档";
            ...
            if (s.contains("вентил") || s.contains("продув") || s.contains("обдув")) {
                ...
                if (grade.equals("MAX")) return z + "座椅通风3档";
                if (grade.equals("MIN")) return z + "座椅通风1档";
            ...
            if (s.contains("массаж") || s.contains("массир")) {
                ...
                if (s.contains("режим") || s.contains("друго") || s.contains("следующ") || s.contains("смени")
                    ...
                    || s.contains("вид массажа")) return z + "换个按摩模式";                        // SWITCH_SEAT_MASSAGE_MODE
                if (n >= 1 && n <= 3) return z + "座椅按摩" + n + "档";                         // SET_SEAT_MASSAGE gear
                if (grade.equals("PLUS")) return z + "按摩强度调大一点";
                if (grade.equals("MINUS")) return z + "按摩强度调小一点";
                if (grade.equals("MAX")) return z + "座椅按摩3档";
                if (grade.equals("MIN")) return z + "座椅按摩1档";
                return (off ? "关闭" : "打开") + z + "座椅按摩";
            }
            if (s.contains("поясниц")) {                                                    // ADJUST_LUMBAR_POSITION
                if (grade.equals("MINUS") || s.contains("слабее") || s.contains("ниже")) return z + "腰部支撑调低一点";
                return z + "腰部支撑调高一点";
            }
            ...
            if (s.contains("сохрани") || s.contains("запомни")) return zoneOf(s).isEmpty() ? "保存当前座椅位置" : "保存" + z + "座椅位置";   // SWITCH_SAVE_SEAT_POSITION
        ...
        if (s.contains("подставк") || s.contains("подножк") || (s.contains("ног") && s.contains("опор"))) {
            ...
            if (s.contains("ниже") || grade.equals("MINUS")) return z + "腿部支撑调低一点";       // ADJUST_LEG_SUPPORT_POSITION
            if (s.contains("выше") || grade.equals("PLUS")) return z + "腿部支撑调高一点";
            return "打开" + z + "脚托";
        }
    ...
    private static String zhClimate(String s, boolean off, String z, String grade, int n) {
        ...
        if (s.contains("обдув лоб") || s.contains("обдув стек") || s.contains("обогрев стек")
            ...
            || (s.contains("подогрев") && (s.contains("стекл") || s.contains("лобов")))
        ...
        if (s.contains("жарко") || s.contains("душно") || s.contains("мне жарко") || s.contains("парилка")
            ...
            || s.contains("пекло") || s.contains("духота")) return z + "温度调低一点";
        if (s.contains("прохладн")) return z + "温度调低一点";
        ...
        if (s.contains("дует") && !s.contains("улиц") && !s.contains("ветер")) {
            if (s.contains("сильн") || s.contains("слишком")) return z + "风量调小一点";
            if (s.contains("слаб") || s.contains("еле")) return z + "风量调大一点";
        }
        // "печка" = heater ("отопление", "обогреватель")
        if (s.contains("печк") || s.contains("отоплен") || s.contains("обогреватель")) {
            if (off) return "关闭空调";
            if (grade.equals("PLUS")) return z + "温度调高一点";
            if (grade.equals("MINUS")) return z + "温度调低一点";
            return "打开制热模式";
        }
        ...
        if ((s.contains("температур") || s.contains("градус") || s.contains("теплее") || s.contains("холодн"))
            ...
            if (grade.equals("MAX")) return z + "温度调到最高";
            if (grade.equals("MIN")) return z + "温度调到最低";
            ...
            String step = halfStep ? "0.5" : s.contains("полтора") ? "1.5" : (td >= 0.5 && td <= 8 && s.contains("на ")) ? tv : null;
            // "теплее на 2 градуса" -> relative step with value
            if (grade.equals("PLUS") || plusWordOf(s)) return step != null ? z + "温度调高" + step + "度" : z + "温度调高一点";
            if (grade.equals("MINUS") || minusWordOf(s)) return step != null ? z + "温度调低" + step + "度" : z + "温度调低一点";
        ...
        if (s.contains("дуй") || s.contains("обдув в") || s.contains("обдув на") || s.contains("поток в")
            ...
            // E07 local NLU accepts FOOT_WINDOW; keep the requested combination.
            if ((s.contains("стекл") || s.contains("лобов")) && s.contains("ног")) return z + "空调吹玻璃吹脚";
        ...
        if (fanCtx && (s.contains("обдув") || s.contains("вентилятор") || s.contains("дуй")
            ...
            if (n >= 1 && n <= 8) return z + "风量调到" + n + "档";
            if (grade.equals("MAX") || s.contains("на полную")) return z + "风量调到最大";
            if (grade.equals("MIN")) return z + "风量调到最小";
            if (grade.equals("PLUS") || plusWordOf(s) || s.contains("усил") || s.contains("быстр")) return z + "风量调大一点";
            if (grade.equals("MINUS") || minusWordOf(s) || s.contains("ослаб") || s.contains("медленн")) return z + "风量调小一点";
        ...
        if (s.contains("климат") || s.contains("кондиц") || s.contains("кондер") || s.contains("кондей")
            ...
            String tv = tempValueOf(s);
            if (tv != null && Double.parseDouble(tv) >= 16 && Double.parseDouble(tv) <= 33) return "把" + z + "温度调到" + tv + "度"; // "кондиционер на 22"
    ...
    private static final java.util.regex.Pattern STOP_WORD = java.util.regex.Pattern.compile("(?<![а-я-])стоп(?![а-я-])");

    private static boolean trunkPause(String s) {
        return s.contains("останов") || s.contains("пауз") || hasWord(s, STOP_WORD);
    }

    /** Exact E07 NLU names. Up/down leaves must never become the generic trunk intent. */
    private static String trunkPart(String s) {
        if (s.contains("свет") || s.contains("ламп") || s.contains("освещ") || s.contains("фонар")) return null;
        if (s.contains("перегородк"))
            return s.contains("стекл") || s.contains("окн") || s.contains("окон") ? "后隔断玻璃" : "后隔断";
        if (s.contains("задн") && s.contains("отсек") && (hasWord(s, ALL_WORD) || s.contains("целиком") || s.contains("полностью")))
            return "全后舱"; // Internal E07Trunk contract: never send to OEM NLU as a substitute.
        if (s.contains("крыш") && !s.contains("крышк")
            && (s.contains("сдвижн") || s.contains("багаж") || s.contains("задн") && s.contains("отсек")))
            return "后舱滑移顶盖"; // Explicit sliding roof, distinct from the unresolved upper-leaf alias.
        if ((s.contains("заднее стекло") || s.contains("заднего стекла") || s.contains("заднему стеклу"))
            && !s.contains("обогре") && !s.contains("подогре") && !s.contains("обдув")
            && !hasWord(s, RIGHT_WORD) && !hasWord(s, LEFT_WORD) && !s.contains("боков") && !s.contains("двер"))
            return "后备窗";
        if (s.contains("капот") || s.contains("фрунк") || (s.contains("багаж") && s.contains("передн"))) return "前备箱";
        boolean upper = s.contains("верхн"), lower = s.contains("нижн");
        boolean leaves = s.contains("створк");
        boolean whole = hasWord(s, ALL_WORD) || s.contains("целиком") || s.contains("полностью");
        // Owner decision 2026-09-12: «весь багажник» and «обе створки» = lower board + sliding roof, no glass.
        boolean both = whole && (leaves || (s.contains("багаж") && !upper && !lower && !s.contains("борт")));
        boolean board = s.contains("борт") && (s.contains("задн") || lower || s.contains("багаж"));
        if (!s.contains("багаж") && !board && !(leaves && (upper || lower || both))) return null;
        if (s.contains("стекл") || s.contains("окн") || s.contains("окон") || s.contains("окош")) return "后备窗";
        if (both) return "天地门";
        if (upper) return "天门";
        if (lower || board) return "地门";
        return "后备箱";
    }

    private static boolean trunkRatioTarget(String part) {
        return "后备箱".equals(part) || "地门".equals(part) || "后舱滑移顶盖".equals(part) || "后隔断玻璃".equals(part);
    }

    private static String trunkGate(String zh) {
        return ALLOW_TRUNK ? zh : unsafeGate(zh);
    }

    private static String zhTrunk(String s, boolean off) {
        String part = trunkPart(s);
        if (part == null) return null;
        // Runtime E07Trunk must consume these exact commands before injectZh: stock DM mishandles PAUSE.
        int percent = percentOf(s);
        if (percent >= 0) {
            String result = part + "打开" + percent + "%";
            return trunkGate(result);
        }
        String action;
        if (trunkPause(s)) action = part.equals("地门") ? "暂停" : "停止";
        else if (s.contains("подним")) action = "关闭";
        else if (s.contains("опуст")) action = "打开";
        else action = off ? "关闭" : "打开";
        String result = action + part;
        return trunkGate(result);
    }
    ...
    private static String zhHudDisplays(String s, boolean off, String grade) {
        ...
        if (s.contains("hud") || s.contains("проекц") || s.contains("хад") || s.contains("худ")) {
            if ((s.contains("процент") || s.contains("%")) && numIn(s) >= 1) return "抬头显示亮度调到" + numIn(s) + "%";
        ...
        if (s.contains("экран") || s.contains("дисплей")) {
            if ((s.contains("процент") || s.contains("%")) && numIn(s) >= 1) return "屏幕亮度调到百分之" + numIn(s);
    ...
    private static String zhComfortModes(String s, boolean off, int n) {
        ...
        if (s.contains("холодильник") || s.contains("морозилк") || s.contains("холодос")) {
            // «температура холодильника, подходящая для белого вина» / «какую поставить» is a question, not a setting
    ...
    private static String zhSoundMedia(String s, boolean off, String grade, int n) {
        ...
        if ((s.contains("громкост") || s.contains("громче") || s.contains("тише")
            ...
            if (s.contains("громче") || grade.equals("PLUS") || plusWordOf(s)) return step + (rel && n > 0 && n <= 40 ? "调大" + n : "调大一点");
            if (s.contains("тише") || grade.equals("MINUS") || minusWordOf(s)) return step + (rel && n > 0 && n <= 40 ? "调小" + n : "调小一点");
            return null;
        }
        ...
        if (track) {
            ...
            // A named song/artist («включи песню снуп дог») has no Chinese form the stock NLU parses; E07 has no
            // backend, so the phrase stays unrecognized instead of leaking a Russian tail into 我想听.
        }
    ...
    static int numIn(String s) {
        ...
        for (String t : w) {
            ...
            else if (t.equals("сто") || t.equals("ста")) v = 100; // «стоп» / «останови» are not a number.
    ...
    static String tempValueOf(String s) {
        java.util.regex.Matcher m = java.util.regex.Pattern.compile("(?<!\\d)(\\d{1,2})[.,](\\d+)(?!\\d)").matcher(s);
